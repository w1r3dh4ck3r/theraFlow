...
class ConversationEngine:
    ...
    async def handle_message(
        ...
        replies = await self._build_prompt(advance_to, session, last_answer=answer)
    ...
    async def _build_prompt(
        ...
        When ``settings.llm_enabled`` is True and a valid API key is configured,
        the prompt text is generated by the LLM for a more natural, empathetic
        tone.  On any LLM failure (timeout, error, empty response), falls back
        to the hardcoded prompt from :data:`STEP_CONFIGS`.

        The step's UI structure (buttons, list rows) is always determined by
        the hardcoded config — the LLM only generates the message text.
        """
        ...
        if config.use_buttons:
            return [OutgoingMessage(text=prompt_text, buttons=config.to_buttons())]

        if config.use_list:
            return [OutgoingMessage(text=prompt_text, list_rows=config.to_list_rows())]

        # For free-text steps, append option hints if the LLM didn't mention them
